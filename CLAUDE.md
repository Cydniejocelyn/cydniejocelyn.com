...
78 is the current state of the work** (section 77 is the rebuild). Work in
progress lives on branch `neon-db` (the site database switched on, not yet
live): `git switch neon-db` before anything. The 2026 rebuild (bright, personal,
luxury look; every page generated by `tools/gen_*.py` over `tools/lux_page.py`
and `assets/css/lux.css`) **launched on 26 September 2026**: branch
`site-2026` was fast-forwarded onto `main`, so `main` is the new site and new
work happens on `main` again. /retreats/arizona/ is still live, unlinked and
noindex until Cydnie says "launch" (section 68). Read HANDOFF before doing
anything. Where this file and HANDOFF disagree, HANDOFF is right and this
file is stale: fix it rather than working around it.
