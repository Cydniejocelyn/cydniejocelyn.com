...
### Status by page
  ...
  | /a-sounding/ | REBUILT, awaiting her review. Artifact DssHAcZhyTcejNQUjtQSJi. Generated by tools/gen_a_sounding.py (section 72) |
...
## 72. 25 September 2026, evening. BRANCH site-2026, AND THE SOUNDING REBUILT.

  * **The rebuild now lives on branch `site-2026`** (commit f6c9b80 and on),
    at her word. `main` is back to efa4a2c = the live site, clean. Make live
    fixes on main (`git switch main`), rebuild work on site-2026. The branch
    is local only: not pushed, so there is no Vercel preview yet.
  * **Shared page builder: `tools/lux_page.py`.** New pages are generated by a
    short `tools/gen_<folder>.py` that supplies only its head copy, JSON-LD,
    FAQ and <main>; header, footer, sprite (vector wordmark) and scripts come
    from index.html. Run generators from tools/ (`cd tools && python3
    gen_a_sounding.py`). Work with me still uses its older standalone
    generator; fold it into lux_page when next touched.
  * **The Sounding (/a-sounding/)**, her answers: free call first, then the
    Sounding for EVERY client before a Build ("every client starts here"
    stays true); keep the name that converts: "The Sounding", subtitled "a
    90-minute strategy session" everywhere. Kept her Felt/Found pairs, the
    one-page deliverable (now drawn as a page), and "Sometimes the answer is
    that you don't need me" (on ink). Cut the water line, "no discovery
    call", and two of the three price blocks. Header marks Work with me as
    current. Reuses the ww- hero/head/steps; new rules carry an sd- prefix.
  * GOTCHA: site.css still styles the OLD page's class names (.sd-book,
    .sd-honest, .sd-page...). New class names must not reuse them; the
    booking section rendered centred until .sd-book became .sd-buy. Grep
    site.css for a class before using it on any rebuilt page.
  * Review artifacts, all cross-linked: home LudmMGuNGvWvzyvBQLN7o8 (v7),
    Work with me 5wqFeCj6sFUxfKttPZf9th (v4), The Sounding
...
## 73. 25 September 2026, evening. ABOUT REBUILT.

  * Her answers: short story first, full story below; faith stays the
    foundation in its own section; the fourteen years become what clients
    buy, in her words: "business development and relationship building and
    creative ideas to life. Seeing a vision before it's completed and
    initiating and completing it"; the best testimonials only.
  * Order: hero (I'm Cydnie, laptop photo from the earlier studio shoot) /
    What I bring (four strengths) / My story (four moments, her words,
    sticky heading on desktop) / What held (faith, verses, on ink) / In
    their words (Tamara, Spencer, Angela, Kristi, Carol, BJB + Melissa's
    video, self-hosted, verified playing) / The rest of me (her list, word
    for word) / Three ways in (Work with me, retreats, Letters) / close.
  * ONE BRIDGE LINE IS MINE, marked in gen_about.py: "That is why I build
    businesses that don't need their owner in the middle of everything. I
    know what it costs." Read it to her; cut it if it is not hers.
  * Removed: Clarify/Reclaim/Build, "Come up", "yours whether or not we ever
    speak again", the Sounding as the only button, and a hero caption I had
    drafted with an invented number ("four states"), caught before review.
  * Uses the earlier studio shoot (cydnie-hero, -window, -sing, -mug): they
...
## 74. 25 September 2026, night. RETREATS HUB REBUILT.

  * tools/gen_retreats.py. Every date fact kept from the old page;
    Arizona stays UNNAMED ("A third date is coming", May 2027, Clarissa)
    until she says launch. Melissa's video and the Costa Rica quotes live
    here now (moved off About). Fixed: "Twice a year" beside three dates,
    old offer names, "Carol Poedel" -> "Carol", "Not for you if" -> "What it
    isn't". Links unchanged: Greece waitlist 6a21d07b..., May list
    cf_id/69fa3c69..., private inquiry cf_id/69fa33ac....
  * Two regressions caught and fixed: `.hv-pill` no longer exists in
    lux.css (the luxury pass dropped it), so pills use `.rx-pill`; and the
    About rewrite had deleted the shared video tile styles (`.au-vid`,
    `.au-play`), now restored as shared rules at the end of lux.css.
...
## 75. 25 September 2026, night. GATLINBURG REBUILT (the live booking page).

  * tools/gen_gatlinburg.py. EVERYTHING A GUEST PAYS AGAINST IS UNCHANGED
    and was checked by count after generating: $2,790 / $1,490, $2,900 /
    $1,600 from 1 November, $500 hold, five monthly payments, and the four
    HoneyBook records (private5monthpymtgatlinburg, gatlinburgpymtfullprivate,
    gatlinburgsharedroom5monthspymtplan, gatlinburgsharedpymtfull), each on
    the room card and again in the booking band. List: cf_id/69fa3c69.
  * Kept verbatim: the 24-photo gallery (tools/gatlinburg_gallery.html,
    site.js rail + lightbox, both verified working), the park figures with
    data-count, Kayla's and Cydnie's bios, the countdown (site.js
    [data-countdown], verified counting) and the days-left tag. The page's
    own share image (house-dusk-1200) is kept: lux_page.render now takes a
    full URL for og_image.
  * Order: hero with the six facts / proof strip / why (deck) / five days /
    the house + both rooms + gallery / what the price covers / where you are
    (on ink) / hosts / guests' words (slider + Melissa) / booking band with
    the countdown / FAQ with the Gatlinburg list. Water lines cut ("You are
    under something", "Come up for air" -> "Five days to breathe in the
