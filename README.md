...
`vercel.json` sends a CSP, HSTS, `X-Frame-Options`, `Referrer-Policy`,
`Permissions-Policy`, `Cross-Origin-Opener-Policy` and `nosniff` on every path. Two things to
know before touching them:

- **`script-src` has no `'unsafe-inline'`.** Each inline script is allowed by sha256, and those
  hashes are generated by `seal_csp()` in `tools/build.py` from the *built* files, after the
  comment strippers run. Never hand-edit them; add the script and rebuild.
- **`check_headers()` fails the build** if a required header goes missing or `script-src` is
  widened with a wildcard. If something is being blocked, read the console and add the specific
  origin — do not reach for `'unsafe-inline'`.

Adding any new third-party embed means adding its origin to the right directive. The three in
use are Google Tag Manager, Adobe Fonts (`use.typekit.net`, `p.typekit.net`) and Flodesk
(`assets.flodesk.com`, `form.flodesk.com`), plus `youtube-nocookie.com` in `frame-src`.
